(function($) {
	...
	if($('.mobile-menu').length){
		...
		$('.mobile-menu .navigation > li .dropdown .dropdown-btn').on('click', function(e) {
			e.preventDefault();
			var target = $(this).parent('.dropdown').children('ul');
			...
			if ($(target).is(':visible')){
				...
				$(this).parent('.dropdown').children('ul').slideToggle(500);
			}
		});
...
// slider
	...
	if($('.wow').length){
		...
		wow.init();
			...
			$('#lang-ul').html(langArray);
			//Set the button value to the first el of the array
			$('.langbtn-select').html(langArray[0]);
			$('.langbtn-select').attr('value', 'en');
			...
			$('#lang-ul li').click(function () {
				...
				var item = '<li><img src="' + img + '" alt="" /><span>' + text +
					'</span><i class="fa-solid fa-sort-down"></i></li>';
				$('.langbtn-select').html(item);
				$('.langbtn-select').attr('value', value);
				$(".lang-dropdwn").toggle();
				//console.log(value);
			});
	
			$(".langbtn-select").click(function () {
				$(".lang-dropdwn").toggle();
			});
			...
			if (sessionLang) {
				...
				$('.langbtn-select').html(langArray[langIndex]);
				$('.langbtn-select').attr('value', sessionLang);
			} else {
